...
class KezMenuEffectAble(object):
    ...
    def _updateEffects(self, time_passed):
        ...
        for name, params in self._effects.items():
            self.__getattribute__("_effectupdate_%s" % name.replace("-", "_"))(
                time_passed
            )
    ...
    def _effectinit_enlarge_font_on_focus(self, name, **kwargs):
        ...
        kwargs["raise_font_ps"] = (
            kwargs["enlarge_factor"] / kwargs["enlarge_time"]
        )  # pixel-per-second
    ...
    def _effectupdate_enlarge_font_on_focus(self, time_passed):
        ...
        for o in self.options:
            ...
            elif o["raise_font_factor"] != 1.0:
                # decrease
                if o["raise_font_factor"] > 1.0:
                    o["raise_font_factor"] -= fps * time_passed
                elif o["raise_font_factor"] < 1.0:
                    o["raise_font_factor"] = 1.0
    ...
    def _effectinit_raise_line_padding_on_focus(self, name, **kwargs):
        ...
        if "padding" not in kwargs:
            kwargs["padding"] = 10
        kwargs["padding_pps"] = (
            kwargs["padding"] / kwargs["enlarge_time"]
        )  # pixel-per-second
